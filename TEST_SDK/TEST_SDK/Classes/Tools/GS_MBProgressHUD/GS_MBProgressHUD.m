...
@implementation GS_MBProgressHUD
...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	GS_MBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	GS_MBProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
		return YES;
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
	NSArray *huds = [GS_MBProgressHUD allHUDsForView:view];
	for (GS_MBProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
+ (MB_INSTANCETYPE)HUDForView:(UIView *)view {
	...
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:self]) {
			return (GS_MBProgressHUD *)subview;
		}
	}
	return nil;
}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.animationType = GS_MBProgressHUDAnimationFade;
		self.mode = GS_MBProgressHUDModeIndeterminate;
		self.labelText = nil;
...
- (void)show:(BOOL)animated {
    NSAssert([NSThread isMainThread], @"GS_MBProgressHUD needs to be accessed on the main thread.");
...
- (void)showUsingAnimation:(BOOL)animated {
	...
	if (animated && animationType == GS_MBProgressHUDAnimationZoomIn) {
		self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
	} else if (animated && animationType == GS_MBProgressHUDAnimationZoomOut) {
	...
	if (animated) {
		...
		self.alpha = 1.0f;
		if (animationType == GS_MBProgressHUDAnimationZoomIn || animationType == GS_MBProgressHUDAnimationZoomOut) {
...
- (void)hideUsingAnimation:(BOOL)animated {
	...
	if (animated && showStarted) {
		...
		// in the done method
		if (animationType == GS_MBProgressHUDAnimationZoomIn) {
			self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(1.5f, 1.5f));
		} else if (animationType == GS_MBProgressHUDAnimationZoomOut) {
			self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
		}
...
- (void)updateIndicators {
	...
	if (mode == GS_MBProgressHUDModeIndeterminate) {
	...
	else if (mode == GS_MBProgressHUDModeDeterminateHorizontalBar) {
		// Update to bar determinate indicator
		[indicator removeFromSuperview];
		self.indicator = MB_AUTORELEASE([[MBBarProgressView alloc] init]);
		[self addSubview:indicator];
	}
	else if (mode == GS_MBProgressHUDModeDeterminate || mode == GS_MBProgressHUDModeAnnularDeterminate) {
		if (!isRoundIndicator) {
		...
		if (mode == GS_MBProgressHUDModeAnnularDeterminate) {
	...
	else if (mode == GS_MBProgressHUDModeCustomView && customView != indicator) {
		// Update custom view indicator
		[indicator removeFromSuperview];
		self.indicator = customView;
		[self addSubview:indicator];
	} else if (mode == GS_MBProgressHUDModeText) {
		[indicator removeFromSuperview];
		self.indicator = nil;
	}
}
