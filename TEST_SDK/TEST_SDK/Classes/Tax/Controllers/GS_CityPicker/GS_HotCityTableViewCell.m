...
@implementation GS_HotCityTableViewCell
- (void)setHotCityArray:(NSArray *)hotCityArray
{
    ...
    for (int i=0; i<_hotCityArray.count; i++) {
        ...
        button.backgroundColor = GS_Background_Color;
...
{
    if ([self.delegate respondsToSelector:@selector(clickGS_HotCityTableViewCell:)]) {
        [self.delegate clickGS_HotCityTableViewCell:button];
    }else
    {
        NSLog(@"点击热门城市代理不响应");
    }
